		...
		<form name="monForm" action="" method="post">
			...
			<div class="form-line mb-4">
				<label class="form-label">Date de naissance *</label> <input
					class="form-control" type="date" name="naissance"
					placeholder="dd/mm/yyyy" required value="<%=p.getNaissance()%>">
			</div>

			<div class="form-line mb-4">
				<label class="form-label">Adresse *</label> <input
					class="form-control" type="text" name="adresse"
					placeholder="Votre adresse" required value="<%=p.getAdresse()%>">
			</div>

			<div class="form-line mb-4">
				<label class="form-label">Pays *</label> <select class="form-select"
					name="pays" required id="pays" onchange="load()">
					<option
						<%if (p.getPays().equals("France"))
	out.println("selected");%>>France</option>
					<option
						<%if (p.getPays().equals("Belgique"))
	out.println("selected");%>>Belgique</option>
					<option
						<%if (p.getPays().equals("Etats Unis"))
	out.println("selected");%>>Etats
						Unis</option>
				</select>
			</div>
			...
			<div class="form-line mb-4">
				...
				<div id="cities-content" style="width: 59%; float: right">
					<select class="form-select" name="ville" required id="ville">
						...
						%>
						<option
							<%if (p.getVille().equals(v.getNom()))
	out.println("selected");%>><%=v.getNom()%></option>
						<%
						}
						%>
					</select>
